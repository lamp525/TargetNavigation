...
{
    public class TagSearchBLL
    {
        /// <summary>默认显示的历史检索记录数量</summary>
        ...
        public async Task SaveRecentSearchTag(int userId, string[] tags)
        {
            await TagUtility.SaveRecentSearchTagAsync(userId, tags);
        }
        ...
        {
            var planIds = TagUtility.GetTagSearchResult(tagNames, EnumDefine.TagType.Plan);
            var loopIds = TagUtility.GetTagSearchResult(tagNames, EnumDefine.TagType.LoopPlan);
        ...
        /// <param name="tagNames"></param>
        /// <returns></returns>
        private List<ObjectiveSearchResultModel> GetObjectiveSearchResult(int userId, string[] tagNames)
        {
            var objectiveIds = TagUtility.GetTagSearchResult(tagNames, EnumDefine.TagType.Objective);
        ...
        /// <param name="tagNames"></param>
        /// <returns></returns>
        private List<NewsSearchResultModel> GetNewsSearchResult(int userId, string[] tagNames)
        {
            var newsIds = TagUtility.GetTagSearchResult(tagNames, EnumDefine.TagType.News);
        ...
        /// <param name="tagNames"></param>
        /// <returns></returns>
        private List<DocumentSearchResultModel> GetDocumentSearchResult(int userId, string[] tagNames)
        {
            var userDocIds = TagUtility.GetTagSearchResult(tagNames, EnumDefine.TagType.UserDocument);
            var companyDocIds = TagUtility.GetTagSearchResult(tagNames, EnumDefine.TagType.CompanyDocument);
        ...
        /// <returns></returns>
        private List<ObjectiveSearchResultModel> FiltrateObjectiveSearchResult(int userId, List<int> objectiveIds)
        {
            var objectiveInfoList = new List<ObjectiveSearchResultModel>();
            ...
            using (TargetNavigationDBEntities db = new TargetNavigationDBEntities())
            {
                ...
                objectiveInfoList = (from o in db.tblObjective
                                     ...
                                     select new ObjectiveSearchResultModel
                                     {
                                         ...
                                         bigImage = string.IsNullOrEmpty(create.bigImage) ? ConstVar.DefaultUserHead : (ConstVar.WebHostURL + ConstVar.HeadImageUpLoadPath + "/" + create.bigImage),
        ...
        /// <returns></returns>
        private List<NewsSearchResultModel> FiltrateNewsSearchResult(int userId, List<int> newsIds)
        {
            var newsInfoList = new List<NewsSearchResultModel>();
            ...
            using (TargetNavigationDBEntities db = new TargetNavigationDBEntities())
            {
                newsInfoList = (from news in db.tblNews
                                ...
                                select new NewsSearchResultModel
                                {
                                    newId = news.newId,
                                    title = news.title,
                                    directoryName = directory.directoryName,
                                    createUserName = db.tblUser.Where(x => x.userId == news.createUser).Select(x => x.userName).FirstOrDefault(),
                                    createTime = news.createTime,
                                    updateTime = news.updateTime,
                                    isNotice = news.notice
                                }).ToList();
            }
        ...
        /// <returns></returns>
        private List<DocumentSearchResultModel> FiltrateDocumentSearchResult(int userId, List<int> userDocIds, List<int> companyDocIds)
        {
            var docInfoList = new List<DocumentSearchResultModel>();
            ...
            using (TargetNavigationDBEntities db = new TargetNavigationDBEntities())
            {
                ...
                if (companyDocIds.Count > 0)
                {
                    ...
                    foreach (var doc in docList)
                    {
                        ...
                        {
                            var upPower = (int)EnumDefine.FolderAuthPower.FullControl;
                            ...
                            foreach (var id in sortedFolderList)
                            {
                                var authPower = (from fa in db.tblFolderAuth
                                                 ...
                                                 where ((fa.type == (int)EnumDefine.FolderAuthType.Organization && orgIds.Contains(ar.targetId)) ||
                                                           (fa.type == (int)EnumDefine.FolderAuthType.Station && stationIds.Contains(ar.targetId.Value)) ||
                                                           (fa.type == (int)EnumDefine.FolderAuthType.User && userId == ar.targetId)) &&
                                                           fa.documentId == id
                                                 select fa.power
                                                         ).FirstOrDefault();
                                ...
                                if (upPower <= (int)EnumDefine.FolderAuthPower.NoAccese) break;
                            }

                            if (upPower > (int)EnumDefine.FolderAuthPower.NoAccese)
                            {
                                var docInfo = new DocumentSearchResultModel
                                {
                ...
                if (userDocIds.Count > 0)
                {
                    ...
                    var userDocList = (from ud in db.tblUserDocument
                                       ...
                                       select new DocumentSearchResultModel
                                       {
                                           ...
                                           power = (int)EnumDefine.FolderAuthPower.FullControl
                                       }).ToList();
                    ...
                    var otherSharedDocList = (from ud in db.tblUserDocument
                                              ...
                                              select new DocumentSearchResultModel
                                              {
                                                  ...
                                                  updateTime = ud.updateTime,
                                                  withShared = ud.withShared,
                                                  power = (int)EnumDefine.FolderAuthPower.DownloadOnly
                                              }).ToList();
