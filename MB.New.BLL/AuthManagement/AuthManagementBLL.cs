...
namespace MB.New.BLL.AuthManagement
{
    public class AuthManagementBLL
    {
        #region 变量
        ...
        /// <returns></returns>
        public List<AuthShift> GetAuthList(TargetNavigationDBEntities db,string authName = null)
        {
        ...
        public void SaveAuth(AuthShift authModel,TargetNavigationDBEntities db)
        {
        ...
        /// </summary>
        /// <param name="id"></param>
        public void DeleteAuth(int id,TargetNavigationDBEntities db)
        {
        ...
        public void AuthShift(AuthShift authShift,TargetNavigationDBEntities db)
        {
                ...
                if (authShift.FlowData.Count() != 0)
                {
                    ...
                    {
                        foreach (var item in flowList.Where(c => c.flowU.status == (int)(EnumDefine.UserFormStatus.unSubmit)).ToList())
                        {
                    ...
                    if (authShift.FlowData.Contains(2))
                    {
                        foreach (var item in flowList.Where(c => c.fd.alreadyRead == (int)EnumDefine.FormDuplicateStatus.Ready).ToList())
                        {
                            var form = db.tblFormDuplicate.Where(c => c.formId == item.fd.formId && c.userId == authShift.turnUserId && c.nodeId == item.fd.nodeId).FirstOrDefault();
                            if (form != null)
                            {
                    ...
                    {
                        var fd = flowList.Where(c => c.flowU.status == (int)EnumDefine.UserFormStatus.flowing && (c.fd.alreadyRead == (int)EnumDefine.FormDuplicateStatus.Approval || c.fd.alreadyRead == (int)EnumDefine.FormDuplicateStatus.Submit || c.fd.alreadyRead == (int)EnumDefine.FormDuplicateStatus.Countersign)).ToList();
